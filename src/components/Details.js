...
const Details = ({ id }) => {
  ...
  let [videoDetails,setVideoDetails] = useState("");
    ...
    const fetchData = async () => {
        try {
            const response = await fetch(`https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics&key=AIzaSyBQRyqykU6NycYaiHRgjUIIG5OIRIsg_Pw&id=${id}`);
